...
function renderActionPills() {
  ...
  el.innerHTML = `<label class="quick-category-label" for="quick-action-select">Entry Category</label><select id="quick-action-select" class="quick-action-select" onchange="selectActionType(this.value)">${types.map(t => `<option value="${escapeAttr(t.id)}" ${t.id === selectedActionTypeId ? 'selected' : ''}>${escapeHtml(t.icon)} ${escapeHtml(t.label)}</option>`).join('')}</select>`;
}
...
function selectActionType(id) {
  ...
  const select = document.getElementById('quick-action-select');
...
function renderDashboard() {
  ...
  const formattedRegistration = normalizeRegistrationNumber(vehicle.registrationNumber);
  const vehicleDetail = formattedRegistration ? `${vehicle.name} · ${formattedRegistration}` : vehicle.name;
  if (el) el.innerHTML = `<div class="dashboard-card vehicle-card"><h3>MANSKIT Vehicle</h3><div class="dashboard-list vehicle-details"><strong class="vehicle-name">${escapeHtml(vehicleDetail)}</strong></div></div>` + card('Near expiry documents', docs.map(d => ({label:d.type, date:d.expiryDate})), 'All documents are clear for 30 days.', true, 'document-card') + card('Reminders', reminders.map(r => ({label:r.title, date:r.dueDate})), 'No reminders due in the next 30 days.', true, 'reminder-card') + `<div class="dashboard-card ownership-card"><h3>Vehicle ownership</h3><div class="dashboard-list">${escapeHtml(ownership)}</div></div>`;
}
...
function renderProfileScreen() {
  ...
  document.getElementById('cover-card').innerHTML = `
    ...
    <div class="cover-caption">
      ...
      <span class="hint-text">${capitalizeFirst(vehicle.type)} · ${vehicle.registrationNumber ? escapeHtml(normalizeRegistrationNumber(vehicle.registrationNumber)) + ' · ' : ''}${vehicle.ownership?.mode === 'partnership' ? 'Partnership' : 'Single owner'}</span>
    </div>
  `;
...
function normalizeRegistrationNumber(str) {
  const compact = (str || '').toString().toUpperCase().replace(/[^A-Z0-9]/g, '');
  if (!compact) return '';
  // Common Indian registration pattern: MH 12 AB 1234.
  const indian = compact.match(/^([A-Z]{2})(\d{1,2})([A-Z]{1,3})(\d{1,4})$/);
  if (indian) return `${indian[1]} ${indian[2]} ${indian[3]} ${indian[4]}`;
  // Readable fallback for values such as RTB9918 or ABC123.
  const lettersThenDigits = compact.match(/^([A-Z]+)(\d+)$/);
