...
const DB = {
  ...
  _migrate() {
    ...
    Object.values(d.vehicles || {}).forEach(v => {
      ...
      const registrationCompact = v.registrationNumber.toUpperCase().replace(/[^A-Z0-9]/g, '');
      const indianRegistration = registrationCompact.match(/^([A-Z]{2})(\d{1,2})([A-Z]{1,3})(\d{1,4})$/);
      const lettersThenDigits = registrationCompact.match(/^([A-Z]+)(\d+)$/);
      v.registrationNumber = indianRegistration
        ? `${indianRegistration[1]} ${indianRegistration[2]} ${indianRegistration[3]} ${indianRegistration[4]}`
        : lettersThenDigits ? `${lettersThenDigits[1]} ${lettersThenDigits[2]}` : registrationCompact;
